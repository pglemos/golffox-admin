...
const CompaniesPage = ({ companies, glassClass, tokens, isLight }: CompaniesPageProps) => {
  ...
  const filteredCompanies = useMemo(() => {
    ...
    return companies.filter((company) => {
      ...
      const matchesSearch =
        filtroNormalizado.length === 0 ? true : nomeMatch || contatoMatch || enderecoMatch || cnpjMatch
                ...
                filteredCompanies.map((company) => (
                  <tr key={company.id} className={rowHoverClass}>
                    ...
                    <td className="px-4 py-3">
                      <span
                        className={`inline-flex items-center gap-1 rounded-full px-3 py-1 text-xs font-semibold ${statusBadgeClasses(
                          company.status
                        )}`}
                      >
